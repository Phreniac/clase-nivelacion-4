...
//const haciaejercicioElements = document.querySelectorAll('.haciaejercicio');



//const button_next = document.getElementById('button_next');

//function irEjercicio2() {
...
//  } 
  ...
  const tasks = [
    ...
    '<p>2. El color debe quedar momentáneamente, de manera que quede seleccionado</p>',
  ...
  haciaejercicioElements.forEach((element) => {
    element.addEventListener('mouseenter', () => {
      if (!element.classList.contains('seleccionado')) {
        const randomColor = lista_colores[Math.floor(Math.random() * lista_colores.length)];
    ...
    element.addEventListener('click', () => {
      ...
      if (seleccionados === haciaejercicioElements.length) {
